...
- *Always include an idempotency key on every mutating Stripe API call* (POST/DELETE — charges, payment intents, subscriptions, refunds, customers, invoices, etc.). The key must be **stable per logical operation** — derive it deterministically from business IDs (`orderId`, `webhookEventId`, `customerId + priceId`) rather than calling `crypto.randomUUID()` fresh per request. A per-request UUID defeats idempotency for concurrent double-clicks and client retries that don't re-use the same UUID. Pass it as the `Idempotency-Key` request header when using raw fetch, or as the `idempotencyKey` option in the SDK. GET requests are inherently idempotent and do not require a key.

- *Factory runs on Cloudflare Workers — app code must use `@latimer-woods-tech/stripe`*, never the raw `stripe` npm package directly. The `@latimer-woods-tech/stripe` package wraps the Stripe SDK with `Stripe.createFetchHttpClient()` so it works inside Workers without Node.js built-ins. For webhook signature verification, use `validateWebhook` from `@latimer-woods-tech/stripe`, which uses the Web Crypto API (`crypto.subtle`) internally — not `stripe.webhooks.constructEvent`, which requires Node.js `crypto`. Never read `process.env`; always use Worker env bindings (`env.STRIPE_SECRET_KEY`, etc.).
