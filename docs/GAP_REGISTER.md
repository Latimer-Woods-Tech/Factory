...
| G2 | Aggregator + sync scripts have no unit tests | **closed** | @sauna + @factory-cross-repo[bot] | stage-1 | Phase 2 shipped via PR [#6f9dc032](https://github.com/Latimer-Woods-Tech/Factory/commit/6f9dc032c573b7c1bb866dc35eb1b9aeb5151354) — 129 tests, 69.4% combined coverage on the 3 scripts (86% on aggregate_completion, 69% on init-matrix-issues, 46% on sync_labels_to_matrix). Tested: parse logic (matrix / labels / rows), HTTP mocking via fetch_fn injection, error paths (404/5xx/invalid JSON), status overlay application, render + Pushover output, label reconciliation, main() error handling (missing token, config load), Stripe/Sentry/GitHub integration stubs. Phase 1 (basic parsing) + Phase 2 (HTTP mocking + orchestration) complete. Coverage of complex I/O paths (git operations, PR creation) deferred to stage-2 by priority. |
...
| G5 | Claude reviewer not calibrated | **closed** (tuning complete) | @sauna | stage-1 | Shadow run 2026-05-22: 50 merged PRs analyzed, results 98% accuracy (0% FP, 2% FN), ready for Phase 1 deployment. Tuning: CLAUDE.md clarified that Hard Constraints apply to production code (Cloudflare Workers runtime) only, not to GitHub Actions scripts (`.github/scripts/**/*.mjs`). Bot should respect this scope boundary. Phase 2 enforcement ready by 2026-06-05. |
...
| G37 | `CF_API_TOKEN` and `LATIMERWOODS_SENTRY_AUTH` lack scopes for billing/stats endpoints | **closed** (Sentry fixed, CF pending) | @adrper79-dot | stage-2 | Sentry fixed 2026-05-22 (commit 38bb4bf3): reordered token fetch to use `FACTORY_SENTRY_API` (has stats scope) before `LATIMERWOODS_SENTRY_AUTH` in both cost-observability and revenue-digest workflows. Verified HTTP 200 + actual stats retrieval. Cost observability line now operational. CF `billing/profile` still 403 (scope needed on CF_API_TOKEN) — lower priority, scoped for Stage 2. |
