...
| G34 | No PostHog funnel definitions for monetization paths | **in-progress** | @sauna | stage-2 | Closure in [PR #809](https://github.com/Latimer-Woods-Tech/Factory/pull/809) via [`docs/marketing/LIFECYCLE.md`](./marketing/LIFECYCLE.md) — per-cell funnels defined for selfprime:practitioner, selfprime:consumer, cypher:practitioner, factory:internal; checked-in JSON in `docs/marketing/funnels/` once [PR 3e supervisor worker](./marketing/pr3-briefs/3e-supervisor-worker.md) ships. Original spec: pre-built funnels in `@lwt/analytics`: signup→first-action→paid→renewal→day-30-retention. |
