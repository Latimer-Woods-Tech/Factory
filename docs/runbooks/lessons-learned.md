...
## Stripe Connect session (June 2026 — HumanDesign PR #630)

### Literal strings in EXACT_ROUTES (`Map.get`) never match parametric paths — silently 404
`EXACT_ROUTES` uses `Map.get(method + ' ' + path)` for an O(1) lookup against the literal request path. Adding `'POST /api/client/book/:slug'` to EXACT_ROUTES means the map key is the string `POST /api/client/book/:slug` — it will never equal the actual request path `POST /api/client/book/jane-doe`. The feature appeared to build and deploy clean; every booking attempt returned 404 with no error log. **Fix:** parametric routes must go in `PATTERN_ROUTES` with a regex — `[/^\/api\/client\/book\/([a-z0-9-]+)$/, 'POST', 1, handler]`. **Lesson:** any new route with `:param` in the path belongs in `PATTERN_ROUTES`, full stop. CI cannot catch this because the test suite mocks the router.

### `ON CONFLICT (col) DO NOTHING` is a no-op when `col` IS NULL
In Postgres, `NULL != NULL` for UNIQUE constraint purposes — two rows with `NULL` in a UNIQUE column do NOT conflict. An idempotency guard of `ON CONFLICT (stripe_transfer_id) DO NOTHING` does nothing if `stripe_transfer_id` is NULL at insert time (which it always is for a freshly-paid booking — the transfer settles async later). Each webhook retry inserts a duplicate row silently. **Fix:** a partial unique index on a column that IS populated at insert time: `CREATE UNIQUE INDEX ce_booking_session_uniq ON connect_earnings(booking_session_id) WHERE booking_session_id IS NOT NULL;` then `ON CONFLICT (booking_session_id) DO NOTHING`. **Lesson:** before writing an `ON CONFLICT` guard, check whether the conflict column can ever be NULL at insert time.

### The success_url page must exist before the checkout session — a missing page silently breaks every payment
`stripe.checkout.sessions.create({ success_url: 'https://selfprime.net/book/success?...' })` succeeds even if `/book/success` returns 404. Stripe redirects the user there after payment regardless; the user lands on a 404. The payment went through, the webhook fired, the DB updated correctly — only the user experience was broken. The backend was completely correct and CI was green. **Lesson:** before adding any new `success_url` or `cancel_url` to a Stripe checkout, confirm the target static file exists. Check `client/public/` for an `.html` file at that path before wiring the backend.

### `account.application.deauthorized` is the only event that fires when a practitioner disconnects
Stripe sends `account.updated` when capabilities change (e.g. charges_enabled flips to false after a restriction), but it does NOT fire when a user revokes your application's access via the Stripe dashboard. Only `account.application.deauthorized` fires for that case. Without a handler, a disconnected practitioner keeps `status='active', charges_enabled=true` in your DB indefinitely — clients can still see them as bookable even though all charges will fail. **Fix:** add `case 'account.application.deauthorized'` that sets `status='inactive', charges_enabled=false, payouts_enabled=false`.

### Email profile links must use `slug`, not `display_name` — even after HTML-escaping
`htmlEsc(display_name)` in an email href produces `/p/John%20Doe` (or in HTML attribute context, `/p/John Doe`), both of which 404 because routes are keyed by the URL-safe `slug` field (`john-doe`). The bug survives any amount of escaping because the root cause is using the wrong field, not an encoding problem. **Lesson:** any URL built in server-side code (email templates, redirects, generated links) must use the slug/ID, not the human-readable display name. When fetching data for emails, explicitly SELECT and pass the `slug` field; don't assume `display_name` is URL-safe.

### `UPDATE … RETURNING col` eliminates a round-trip SELECT for data you just wrote
A common pattern is to `UPDATE` a row then immediately `SELECT` the same row to get fields needed for downstream work (e.g. sending an email). The two queries are serializable and the data is deterministic — there is no race. Use `RETURNING col1, col2` on the UPDATE itself: `UPDATE booking_sessions SET status='paid'… WHERE id=$3 RETURNING preferred_time, notes`. One fewer DB round-trip per request. When you also need data from a separate table (e.g. practitioner slug from `practitioners`), combine that into a `Promise.all` with the user lookup rather than a third sequential await.

### On Windows, `git checkout` does not persist working-tree state across separate Bash invocations
The Claude Code Bash tool resets `CWD` to the project root after each call; each call is a fresh shell. More critically, on Windows the Read tool caches file content at the path it was first read — after a `git checkout` to a different branch, the Read tool may still serve the pre-checkout content from its cache. **Consequence:** an `Edit` call to `repo/workers/src/handlers/file.js` while the working tree is on branch A will write to branch A's file, even if you intended to edit branch B's version. **Fix:** use `git worktree add ../repo-branch-name branch-name` to create an isolated working directory for the target branch, then use the worktree path exclusively. Verify actual disk state with `bash cat` or `git show branch:path` rather than the Read tool when there is any branch-switch ambiguity.
