...
| `copilot-pr-poller.yml` | Copilot PR Poller | cron `*/10 * * * *`, manual | ✅ 10/10 | replaces brittle event cascade for Copilot PRs |
...
| **`copilot-swe-agent`** (per-user GitHub Copilot identity) | Authors Yellow-tier sub-PRs (e.g. `copilot/sub-pr-*`, `copilot/fix-*`). Auto-approved by `copilot-auto-approve.yml` when both LLM judges agree. | Owns `copilot/*` branches. Touched by `copilot-auto-approve.yml`, `copilot-pr-poller.yml`, `auto-merge-approved-prs.yml`. | Driven by **individual Copilot subscription** — org plan currently shows `"filled_seats":1, "seats":1` with no Copilot business seats (only base team plan). | Coding-agent toggle had to be flipped on the per-repo settings page today; org-level Copilot seats config is still effectively unconfigured for SWE Agent. Means if the individual seat lapses, the autonomous loop stops. |
