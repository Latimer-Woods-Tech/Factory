...
function buildTemplate(parsed) {
  ...
  // slot_defaults: { [slotName]: defaultValue } — used by enforceSlotSchema as
  // a fallback when LLM extraction returns null or fails validation. Lets a
  // template ship a real PR with placeholder content when the issue body
  // doesn't contain enough signal to extract a slot. The default must itself
  // pass the slot's validator (checked at build time below).
  const slotDefaults = {};
  // slot_descriptions: { [slotName]: humanReadableIntent } — exposed to the LLM
  // by extractSlots() so it knows the SHAPE and INTENT of each slot, not just
  // the name. Lets the LLM synthesize generative content (TypeScript, SQL, test
  // bodies) for slots the issue body doesn't literally contain.
  ...
  for (const slot of (parsed.slots ?? [])) {
    ...
    if (slot.name && slot.default !== undefined && slot.default !== null) {
      // If both a validator and default exist, the default must pass the validator —
      // otherwise the fallback path itself would be nulled, which is a build-time bug.
      if (slot.validator && typeof slot.default === 'string') {
        try {
          if (!new RegExp(slot.validator).test(slot.default)) {
            throw new Error(`[${parsed.id}] slot "${slot.name}" default ${JSON.stringify(slot.default)} does not satisfy its own validator /${slot.validator}/`);
          }
        } catch (e) {
          if (e.message.startsWith(`[${parsed.id}]`)) throw e;
          // Validator regex itself was malformed — already caught above
        }
      }
      slotDefaults[slot.name] = slot.default;
    }
    if (slot.name && typeof slot.description === 'string' && slot.description.trim()) {
      slotDescriptions[slot.name] = slot.description.trim();
    }
  }
...
// (slot_names, slot_validators, slot_defaults, slot_descriptions, step_intents, pr_files are not part of the Worker Template type)
const workerTemplates = templates.map(({ slot_names, slot_validators, slot_defaults, slot_descriptions, step_intents, pr_files, ...t }) => t);
